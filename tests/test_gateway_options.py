"""The existing Miot cloud form stays separate from gateway opt-in."""
...
def test_gateway_is_off_by_default_and_off_needs_no_other_integration():
    async def run():
        flow = _flow()
        form = await config_flow.OptionsFlowHandler.async_step_gateway(flow)
        assert form['step_id'] == 'gateway'
        assert form['data_schema']({})[CONF_SCENE_GATEWAY_MODE] == 'off'
        saved = await config_flow.OptionsFlowHandler.async_step_gateway(
            flow, {CONF_SCENE_GATEWAY_MODE: 'off'},
        )
...
def test_reuse_requires_official_integration_and_independent_prompts_auth(monkeypatch):
    async def run():
        flow = _flow()
        result = await config_flow.OptionsFlowHandler.async_step_gateway(
            flow, {CONF_SCENE_GATEWAY_MODE: 'reuse'},
        )
        ...
        result = await config_flow.OptionsFlowHandler.async_step_gateway(
            flow, {CONF_SCENE_GATEWAY_MODE: 'independent'},
        )
